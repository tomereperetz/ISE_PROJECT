...
public class Polygon extends Geometry {
	...
	protected List<GeoPoint> findGeoIntersectionsHelper(Ray myRay, double maxDistance) {
		...
		for (Point p : vertices)
			vectors.add(p.subtract(myRay.getP0()));
		...
		for (Vector n : normals) {
			if (alignZero(n.dotProduct(myRay.getDir())) >= 0)
				Negative = false;
			if (alignZero(n.dotProduct(myRay.getDir())) <= 0)
				Positive = false;
		}

		List<GeoPoint> result = plane.findGeoIntersections(myRay, maxDistance);
		if (result != null)
	...
	protected void CreateBoundingBox() {
		...
		for (Point point : vertices) {
			double x = point.getXyz().getX();
			double y = point.getXyz().getY();
			double z = point.getXyz().getZ();
